...
export const Footer = () => {
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            ...
            } catch (e) { if (typeof console !== 'undefined' && console.debug) console.debug('[HUD] Cached quota read error:', e?.message || e); }
...
function _parseBuckets(buckets, isPro) {
    ...
    // Pick the MOST CONSTRAINED bucket: lowest remainingFraction (actual daily usage)
    // Per-minute buckets reset constantly (~100%), daily bucket is what actually scales down
